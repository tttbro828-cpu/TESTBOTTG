...
async def simple_shot(call: CallbackQuery, user_id: int, group: str, animal_name: str, weapon: str, location: str, sneak_bonus: int = 0, animal_group: str = None):
    ...
    if hit_success:
        ...
        # Проверка мутации
        # В simple_shot, после получения rank и до начисления наград:

        # ===== МУТАЦИЯ (БЕРЁМ ИЗ ПАРАМЕТРОВ, НЕ ВЫЧИСЛЯЕМ ЗАНОВО) =====
        mutation_name = None
        # mutation_name уже приходит в параметрах! Добавь его в вызов simple_shot

        # Но для обратной совместимости:
        if group in ["Опасн", "Тяжел", "Титан"]:
            # Проверяем, есть ли мутация в БД для этого животного
            conn = sqlite3.connect("hunt.db")
            cur = conn.cursor()
            # Ищем active battle_state для этого user_id
    ...
    if new_animal_hp <= 0:
        ...
        if mutation_name and mutation_name != "None":
            ...
            if mutation_data:
                coins = int(coins * mutation_data["coins_mult"])
                exp = int(exp * mutation_data["exp_mult"])
                print(f"[MUTATION] {animal_name} [{mutation_name}]: x{coins} монет, x{exp} опыта")        
