...
def calculate_damage_with_ability(weapon_name: str, group: str, location: str, user_id: int, body_part: str = None) -> tuple:
 ...
 # Способность оружия - ИСПРАВЛЕННАЯ ЧАСТЬ
    ...
    if ability and ability_chance > 0:
        ...
        if state and state[0] == 0:
            if random.randint(1, 100) <= ability_chance:
                ...
                # НЕ ОТМЕЧАЕМ КАК ИСПОЛЬЗОВАННУЮ - способность сработала автоматически
                # Она все еще доступна для следующего выстрела
            else:
                # Способность НЕ сработала - тоже НЕ отмечаем как использованную
                pass
...
async def show_battle_menu_in_place(call: CallbackQuery, user_id: int, animal_name: str, group: str, animal_hp: int, player_hp: int, max_hp: int):
    """Показывает меню боя в текущем сообщении (добавляет кнопки к результату)"""
    battle = get_battle_state(user_id)
    if not battle:
        await call.message.edit_text("❌ Ошибка: бой не найден")
        return
    
    animal_max_hp = battle[6] if battle[6] > 0 else ANIMAL_HP.get(group, 100)
    user = ensure_user(user_id)
    location = user[4]
    
    effects = get_battle_effects(user_id)
    curse_active = "curse" in effects
    
    # Формируем текст (тот же, что был, плюс приглашение)
    text = f"🎯 **ПОПАДАНИЕ!** Урон был нанесён\n"
    text += f"❤️ HP животного: {animal_hp}/{animal_max_hp}\n\n"
    text += f"❤️ Твоё HP: {player_hp}/{max_hp}\n"
    
    if curse_active:
        text += f"🧪 **Проклятие!** -60% к попаданию в этом ходу!\n"
    
    text += f"\n📍 Локация: {location}\n"
    text += f"\n⚔️ **ТВОЙ ХОД!** Выбери действие:"
    ...
    if ability_used and ability_used[0] == 0:
        ...
        if ability:
            buttons[0].append(InlineKeyboardButton(text="✨ Способность оружия", callback_data=f"battle_ability|{user_id}"))
    ...
    # Обновляем сообщение - теперь с кнопками
    ...
    for action in actions:
        ...
        if rand <= cumulative:
            ...
            if "убежать" in action_name.lower() or "убегает" in action_name.lower():
                ...
                # Шанс побега
                if random.randint(1, 100) <= 20:  # 20% шанс убежать
            ...
            # Защита (возвращаем hit_mod для штрафа игрока)
            if hit_mod != 0:
...
async def battle_sneak(call: CallbackQuery):
    """Подкрасться (+10% к попаданию, но риск при провале)"""
    hunt_id = int(call.data.split("|")[1])
    ...
    # ✅ ИСПРАВЛЕНО: получаем здоровье ДО проверки
    current_hp, max_hp = get_user_health(user_id)
    
    # Проверяем попадание с бонусом
    hit_success = check_hit(weapon, group, user_id, location)
    
    if hit_success:
        # Успешное подкрадывание
        start_battle(user_id, animal_id, animal_name, group, False, 1, 10)
        # Обновляем сообщение - показываем меню боя
        await show_battle_menu(call, user_id, animal_name, group)
    else:
        # ПРОВАЛ - получаем урон
...
async def show_battle_menu(call: CallbackQuery, user_id: int, animal_name: str = None, group: str = None):
    ...
    # Получаем текущий бой из БД
    battle = get_battle_state(user_id)
    if not battle:
        await call.message.edit_text("❌ Бой не найден")
        return
    
    # Берем данные из battle_state, а не из параметров
    animal_name = battle[2]
    group = battle[3]
    animal_hp = battle[5]
    animal_max_hp = battle[6] if battle[6] > 0 else ANIMAL_HP.get(group, 100)
    
    # Для мелочи и средних - сразу стреляем
...
async def battle_body_part(call: CallbackQuery):
    ...
    _, user_id, body_part = call.data.split("|")
    user_id = int(user_id)
    ...
    cur.execute("UPDATE battle_mods SET hit_bonus = ? WHERE user_id = ?", (ord(body_part[0]) % 100, user_id))
    conn.commit()
    conn.close()
    
    # Переходим к атаке
    await battle_attack(call)
...
async def battle_attack(call: CallbackQuery):
    ...
    if not hit_success:
        ...
        # ✅ ПОЛУЧАЕМ ТЕКУЩЕЕ HP ИЗ БД (оно могло измениться от эффектов burn/decay)
        battle = get_battle_state(user_id)
        animal_current_hp = battle[5] if battle else 0
        animal_max_hp = battle[6] if battle and battle[6] > 0 else ANIMAL_HP.get(group, 100)
        
        # ✅ ЕСЛИ new_animal_hp НЕ None (было попадание ранее), используем его
        if new_animal_hp is not None:
            animal_current_hp = new_animal_hp
        ...
        # ✅ ТЕПЕРЬ ИСПОЛЬЗУЕМ ПРАВИЛЬНОЕ ЗНАЧЕНИЕ
        text = (
            f"{miss_phrase}\n\n"
            f"{animal_msg}\n\n"
    ...
    if group == "Титан" and location in TITAN_ABILITIES:
        ...
        if blocked:
            ...
            battle = get_battle_state(user_id)
            animal_hp = battle[5] if battle else 0
            current_hp, _ = get_user_health(user_id)
            
            text = f"{ability_msg}\n\n{animal_msg}\n❤️ Твоё HP: {current_hp}/{max_hp}\n❤️ HP животного: {new_animal_hp}/{ANIMAL_HP[group]}\n\n⚔️ **ТВОЙ ХОД!**"
            buttons = [[InlineKeyboardButton(text="🔫 Выстрел", callback_data=f"battle_attack|{user_id}")]]
            if group == "Титан":
                buttons.append([InlineKeyboardButton(text="🏃 Бежать (10%)", callback_data=f"battle_escape|{user_id}")])
            else:
    ...
    # Расчёт урона (БЕЗ автоматических способностей)
    damage, _, extra_effect = calculate_damage_with_ability(weapon, group, location, user_id, body_part)
    
    # ПОЛУЧАЕМ ТЕКУЩЕЕ HP ИЗ БД (не из переменной battle, чтобы быть уверенным)
    conn = sqlite3.connect("hunt.db")
    cur = conn.cursor()
    ...
    text += f"❤️ HP животного: {new_animal_hp}/{ANIMAL_HP[group]}\n\n"
    text += f"{animal_msg}\n"
    text += f"❤️ Твоё HP: {current_hp}/{max_hp}\n"
    
    # Для тяжелых и титанов - снова выбор части тела (ПОСЛЕ КАЖДОГО ВЫСТРЕЛА)
    if group in ["Тяжел", "Титан"] and location in BODY_PARTS:
