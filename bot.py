...
async def battle_sneak(call: CallbackQuery):
    ...
    if hit_success:
        ...
        # ✅ ТОЛЬКО ДЛЯ СРЕДНИХ - животное убегает после контратаки
        if group == "Средн":
            if new_hp <= 0:
                await call.message.edit_text(
            ...
            # ДЛЯ ОСТАЛЬНЫХ (Опасн, Тяжел, Титан) - бой продолжается
            if new_hp <= 0:
                await call.message.edit_text(
                    f"🥷 Ты попытался подкрасться к {animal_name}, но провалился!\n"
                    f"💥 Животное контратаковало! -{damage} HP\n"
...
async def show_battle_menu(call: CallbackQuery, user_id: int, animal_name: str = None, group: str = None):
    ...
    animal_name = battle[2]
    group = battle[3]
    # ✅ ИСПРАВЛЕНО: используем актуальное HP из БД
    animal_hp = battle[5]
    ...
    # ✅ ИСПРАВЛЕНО: показываем актуальное HP животного
    text = f"⚔️ **БОЙ!** ⚔️\n\n"
...
async def battle_body_part(call: CallbackQuery):
    ...
    # Сохраняем выбранную часть тела (код = индекс + 1)
    ...
    # ✅ ВАЖНО: ПОЛУЧАЕМ АКТУАЛЬНОЕ СОСТОЯНИЕ БОЯ ПРЯМО СЕЙЧАС
    battle = get_battle_state(user_id)
    if not battle:
    ...
    # ✅ ПОЛУЧАЕМ АКТУАЛЬНОЕ HP ЖИВОТНОГО (оно могло измениться от предыдущих атак)
    animal_current_hp = battle[5]
    animal_max_hp = battle[6] if battle[6] > 0 else ANIMAL_HP.get(group, 100)
