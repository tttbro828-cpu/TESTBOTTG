...
WORLD_EVENTS = {
    "blood_moon": {
        ...
        "story": (
            ...
            "📊 **ЭФФЕКТЫ:**\n"
            "• В Тайге +80% шанс найти Волка\n"
            "• 60% волков — мутация «👻 Призрачный»\n"
            "• В Тайге нельзя найти других животных"
        ),
        "effects": {
            "replace_pool": {"Тайга": {"Опасн": ["Серый волк"]}},
            "force_mutation": {"animal": "Серый волк", "mutation": GHOST_MUTATION_NAME, "chance": 60},
            "search_bonus": {"Серый волк": 80}
        }
    },
    "meteor_shower": {
    ...
    "dragon_invasion": {
        ...
        "story": (
            ...
            "• Дракон даёт x5 монет + 100 осколков"
        ),
        "effects": {
            "titan_bonus": 30,
    ...
    "coronation": {
        ...
        "story": (
            ...
            "📊 **ЭФФЕКТЫ:**\n"
            "• Первый, кто убьёт Титана — получает титул «👑 Король»\n"
            "• +50% шанс найти Титана\n"
            "• Все игроки: +25 HP"
        ),
        "effects": {
...
async def announce_world_event(event_id: str):
    """Разослать объявление о событии в ЛС и чаты"""
    event = WORLD_EVENTS.get(event_id)
    if not event:
        return
...
def get_event_search_bonus(group: str) -> int:
    ...
    if group == "Титан":
        ...
        bonus += effects.get("search_bonus", 0)
    return bonus
