...
def claim_daily_streak(user_id: int):
    ...
    # Получаем награду за текущий день (до 500)
    if new_streak <= 500:
        reward = DAILY_STREAK_REWARDS.get(new_streak, {"coins": 350, "exp": 0, "item": None})
    else:
        # Случайная награда после 500 дня
        random_rewards = [
            {"coins": 5000, "exp": 2000},
            {"coins": 7000, "exp": 3000},
    ...
    # ===== ДОПОЛНИТЕЛЬНЫЕ ПРЕДМЕТЫ (extra_items) =====
...
@dp.message(lambda msg: msg.text and msg.text.lower().startswith("дипкартинка"))
...
async def view_location(call: CallbackQuery):
    ...
    # ... весь код формирования animals_text (без изменений) ...
    ...
    # Текст делаем короче для подписи к фото (ограничение 1024 символа)
    ...
    try:
        ...
        if "message is not modified" in str(e):
            ...
            # Fallback — если что-то с фото, показываем текст
            try:
