...
async def show_battle_menu(call: CallbackQuery, user_id: int, animal_name: str = None, group: str = None):
    ...
    # ✅ ИСПРАВЛЕНО: Для тяжелых и титанов - КНОПКИ выстрела и побега + выбор части тела отдельно
    if group in ["Тяжел", "Титан"]:
        # Сначала показываем кнопки для выбора части тела
        if location in BODY_PARTS:
            body_buttons = []
            for part_name in BODY_PARTS[location].keys():
                # Показываем множитель урона рядом с названием
                part_data = BODY_PARTS[location][part_name]
                body_buttons.append([InlineKeyboardButton(
                    text=f"{part_name} (x{part_data['damage_mult']})",
            ...
            # Кнопка выстрела
            action_buttons.append([InlineKeyboardButton(text="🔫 Выстрел", callback_data=f"battle_attack|{user_id}")])
            
            if ability_used and ability_used[0] == 0:
                weapon = user[3]
                ability, ability_desc, ability_chance = get_weapon_ability(weapon)
                if ability:
                    action_buttons[0].append(InlineKeyboardButton(
            ...
            # Кнопка побега
            if group == "Титан":
                action_buttons.append([InlineKeyboardButton(text="🏃 Бежать (10%)", callback_data=f"battle_escape|{user_id}")])
            else:
                action_buttons.append([InlineKeyboardButton(text="🏃 Бежать (75%)", callback_data=f"battle_escape|{user_id}")])
            
            action_kb = InlineKeyboardMarkup(inline_keyboard=action_buttons)
            
            # Объединяем: сначала выбор части тела, потом действия
            await call.message.edit_text(
                text + "\n\n🎯 **Выбери часть тела для выстрела:**",
...
async def battle_body_part(call: CallbackQuery):
    ...
    # Сохраняем выбранную часть тела в battle_mods (код части тела)
    part_code = 0
    location = ensure_user(user_id)[4]
    if location in BODY_PARTS:
        parts_list = list(BODY_PARTS[location].keys())
        if body_part in parts_list:
            part_code = parts_list.index(body_part) + 1
    ...
    # Показываем сообщение о выбранной части тела
    await call.answer(f"✅ Выбрана цель: {body_part}", show_alert=True)
    
    # Теперь показываем меню с кнопками выстрела и побега
    battle = get_battle_state(user_id)
    if battle:
        user = ensure_user(user_id)
        current_hp, max_hp = get_user_health(user_id)
        ...
        text = f"⚔️ **БОЙ!** ⚔️\n\n"
        text += f"🐾 {battle[2]} ({battle[3]})\n"
        text += f"❤️ HP животного: {battle[5]}/{battle[6] if battle[6] > 0 else ANIMAL_HP.get(battle[3], 100)}\n\n"
        text += f"❤️ Твоё HP: {current_hp}/{max_hp}\n"
