...
def start_battle(user_id: int, animal_id: str, animal_name: str, group_name: str, is_pack: bool = False, pack_size: int = 1, sneak_bonus: int = 0):
    ...
    # Для мелочи и средних - НЕ создаем бой
...
# Проверка способности титана (только для титанов)
    if group == "Титан" and location in TITAN_ABILITIES:
        ability = TITAN_ABILITIES[location]
        ability_name = ability["name"]
        ability_effect = ability["effect"]
        ability_chance = ability["chance"]
        
        if random.randint(1, 100) <= ability_chance:
            if ability_effect == "time_rewind":
                # Атака отбита, но титан не атакует в ответ
                await call.message.edit_text(f"🕐 {ability_name}! Твоя атака отбита!")
                await show_battle_menu(call, user_id, animal_name, group)
                return
            
            elif ability_effect == "water_flow":
                await call.message.edit_text(f"🌊 {ability_name}! Твой выстрел отбит!")
                await show_battle_menu(call, user_id, animal_name, group)
                return
            
            elif ability_effect == "curse":
                add_battle_effect(user_id, "curse", 1, 60)
                # Продолжаем расчет урона с проклятием
                await call.message.edit_text(f"🧪 {ability_name}! Ты проклят! -60% к попаданию в следующем ходу!")
                # Не возвращаемся, продолжаем бой
            
            elif ability_effect == "corruption":
                await call.message.edit_text(f"👻 {ability_name}! Ты не можешь выстрелить в этом ходу!")
                # Ход титана
                animal_damage, animal_msg, animal_ran, _ = animal_action(user_id, group, location)
                if animal_damage > 0:
                    update_health(user_id, -animal_damage)
                await call.message.edit_text(f"👻 {ability_name}!\n\n{animal_msg}")
                await show_battle_menu(call, user_id, animal_name, group)
                return
            
            elif ability_effect == "radiation":
                damage = 30
                update_health(user_id, -damage)
                await call.message.edit_text(f"☢️ {ability_name}! Радиация сжигает тебя! -{damage} HP")
                await show_battle_menu(call, user_id, animal_name, group)
                return
            
            elif ability_effect == "lightning":
                damage = 20
                update_health(user_id, -damage)
                await call.message.edit_text(f"⚡ {ability_name}! Молния ударила в тебя! -{damage} HP")
                await show_battle_menu(call, user_id, animal_name, group)
                return
            
            elif ability_effect == "virus":
                if random.randint(1, 100) <= 40:
                    damage = 50
                    update_health(user_id, -damage)
                    await call.message.edit_text(f"💾 {ability_name}! Вирус заставил выстрелить в себя! -{damage} HP")
                    await show_battle_menu(call, user_id, animal_name, group)
                    return
                # Иначе продолжаем
            
            elif ability_effect == "fire_jet":
                damage = 40
                update_health(user_id, -damage)
                await call.message.edit_text(f"🔥 {ability_name}! Струя огня опалила тебя! -{damage} HP")
                await show_battle_menu(call, user_id, animal_name, group)
                return
            
            elif ability_effect == "singularity":
                if random.randint(1, 100) <= 25:
                    update_health(user_id, -999)
                    end_battle(user_id)
                    await call.message.edit_text(f"🌀 {ability_name}! Сингулярность разорвала тебя на части! Ты погиб!")
                    return
                # Иначе продолжаем
    
    # Расчёт урона (продолжение существующего кода)
...
async def battle_sneak(call: CallbackQuery):
    ...
    if group in ["Мелочь", "Средн"]:
        await simple_shot(call, user_id, group, animal_name, weapon, location, sneak_bonus=10)
        return
...
async def battle_shoot(call: CallbackQuery):
    ...
    if group in ["Мелочь", "Средн"]:
        ...
        weapon = user[3]
        location = user[4]
        await simple_shot(call, user_id, group, animal_name, weapon, location)
        return
...
async def simple_shot(call: CallbackQuery, user_id: int, group: str, animal_name: str, weapon: str, location: str, sneak_bonus: int = 0):
    ...
    if hit_success:
        ...
        if group in ["Опасн", "Тяжел", "Титан"]:
            mutation_name, mutation_data = get_animal_mutation(user_id)
            if mutation_name and mutation_data:
        ...
        # ===== КОЛЛЕКЦИОННЫЕ ПРЕДМЕТЫ (шанс 10%) =====
        collectible_msg = ""
        if random.randint(1, 100) <= 10:
            animal_group = None
            for g, animals in LOCATIONS[location]["animals"].items():
                if animal_name in animals:
                    animal_group = g
                    break
            
            if animal_group and location in LOCATION_COLLECTIBLES and animal_group in LOCATION_COLLECTIBLES[location]:
                items = LOCATION_COLLECTIBLES[location][animal_group]
                if items:
                    collectible = random.choice(items)
                    add_collectible(user_id, location, animal_group, collectible)
...
async def battle_attack(call: CallbackQuery):
    ...
    if new_animal_hp <= 0:
        ...
        if random.randint(1, 100) <= 10:
            animal_group = group
            if location in LOCATION_COLLECTIBLES and animal_group in LOCATION_COLLECTIBLES[location]:
                items = LOCATION_COLLECTIBLES[location][animal_group]
                if items:
                    collectible = random.choice(items)
                    add_collectible(user_id, location, animal_group, collectible)
